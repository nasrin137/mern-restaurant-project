    ...
    return (
        <div className="section-container my-20 relative">
            ...
            <div className="text-left">
                <h1 className="title w-[520px]">Standout Dishes From Our Menu</h1>
            </div>

        {/* arrow button */}
        <div className="md:absolute right-3 top-8 mb-10 md:mr-24">
            <button onClick={()=>slider?.current?.slickPrev()} className="btn p-2 rounded-full ml-5">
                <FaAngleLeft className="h-8 w-8 p-1"></FaAngleLeft>
            </button>
            <button onClick={()=>slider?.current?.slickNext()} 
            className="btn p-2 rounded-full ml-5 bg-green">
                <FaAngleRight className="h-8 w-8 p-1"></FaAngleRight>
            </button>
        </div>
